...
## 2026-09-18 — Интеграция библиотеки shadcn/ui и редизайн открытого дашборда

- **Клонирована библиотека**: `https://github.com/shadcn-ui/ui` в локальную директорию `D:\AI\shadcn-ui`.
- **Назначение**: Полный переход UI дашборда от шаблонного любительского стиля к промышленному стандарту дизайн-системы **shadcn/ui** (темная палитра Zinc, типографика Inter/Geist, строгие CSS-переменные HSL, иконки в стиле Lucide SVG, отсутствие визуального шума и эмодзи).
- **Связанные компоненты**:
  - Дашборд: `web/index.html` (переверстан с компонентами Card, Tabs, Table, Badge, Separator в стандарте shadcn).
  - Сервер предпросмотра: `web/serve.py` (порт 8765).
  - Реестр проектов: обновлен паспорт в `Agents_Memory/projects/hearthai.md`.

---

## 2026-09-18 — Мультиязычность (i18n 7 языков) и конфигурация хостинга на Vercel

### 1. Полнофункциональная мультиязычность (i18n)
- Добавлена поддержка 7 ключевых языков киберспортивного сообщества Hearthstone:
  - **EN** (English) — международный стандарт
  - **RU** (Русский) — базовый язык исследования
  - **ZH** (中文) — крупнейший ладдер и соревновательный регион
  - **KO** (한국어) — азиатский киберспортивный хаб
  - **JA** (日本語) — активное сообщество Masters Tour & Grandmasters
  - **DE** (Deutsch) — крупнейшая европейская соревновательная сцена
  - **ES** (Español) — латиноамериканский и испанский регион
- Все 148 ключей локализации переведены и верифицированы без пропусков (`0 missing keys`).
- Переключение языков происходит на лету без перезагрузки страницы с автосохранением выбора в `localStorage('hs_lang')`.

### 2. Конфигурация для хостинга на Vercel
- Добавлен `vercel.json` в корень репозитория:
  - Настроены чистые URL (`"cleanUrls": true`).
  - Настроен маршрутизатор (`rewrites`) для безусловной отдачи статического дашборда из `/web/index.html`.
  - Внедрены политики безопасности в HTTP-заголовках (`X-Content-Type-Options: nosniff`, `X-Frame-Options: DENY`, `X-XSS-Protection: 1; mode=block`).
- Добавлен `web/vercel.json` на случай деплоя с указанием Root Directory = `web`.
- Добавлен корневой fallback `index.html` с автоматическим редиректом на `/web/index.html`.
- Проверена доступность Vercel CLI (v41.6.2 через `npx vercel`).
- Задокументированы 3 сценария деплоя: GitHub Integration, Vercel CLI и Vercel Drop.

---

## 2026-09-18 — Фиксация стратегического решения: Заморозка UI до модернизации SabberStone

### 1. Архитектурное решение (ADR-004)
- **Контекст**: Открытый веб-дашборд обсерватории полностью закончен, оформлен в строгом стандарте shadcn/ui zinc dark, оснащен 7 языками и готов к публикации на Vercel. Однако текущая страница лишь визуализирует статический срез уже проведенных экспериментов и существующие замороженные сплиты (Schema v2).
- **Проблема**: Дальнейшие доработки дашборда без притока свежих данных и динамических симуляций не несут практической ценности для развития модели. Человеческий датасет реплеев (1 041 матч) конечен и страдает от ковариантного сдвига (Action Collapse на редких классах и отсутствие негативных веток выбора).
- **Решение**:
  1. Работы над дашбордом временно **заморозить** на текущем стабильном этапе.
  2. Все созданные наработки (shadcn дашборд, i18n на 7 языков, конфиги Vercel) закоммитить и отправить в удаленный репозиторий GitHub.
  3. Переключить инженерный фокус на **модернизацию и настройку симулятора `D:\AI\SabberStone`**:
     - Аудит поддерживаемых карт и механик C# .NET ядра.
     - Разработка моста / CLI / IPC генерации синтетических партий.
     - Формирование контрфактических негативных траекторий для распутывания Action Collapse и построения preference dataset под DPO / Negative Self-Distillation (NSD).
  4. К расширению дашборда вернуться только после получения динамических симуляций и генерации DPO-пар из SabberStone.
